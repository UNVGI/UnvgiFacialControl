...
| `AnalogBindingDemoActions.inputactions` | 右スティック X/Y と ARKit `jawOpen` 用の `InputActionAsset`。値変換は InputAction の `processors` 文字列に埋め込み |
...
| Action | processors |
|--------|-----------|
| `RightStickX` | `analogDeadZone(min=0.1,max=1)` → `analogScale(factor=45)` → `analogClamp(min=-45,max=45)` |
| `RightStickY` | `analogDeadZone(min=0.1,max=1)` → `analogInvert` → `analogScale(factor=30)` → `analogClamp(min=-30,max=30)` |
...
- **右スティック** で LeftEye / RightEye の Euler が変化することを確認 (`analogDeadZone` → `analogScale` → `analogClamp` 経由で値変換される)
