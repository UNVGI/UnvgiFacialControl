...
namespace Hidano.FacialControl.Samples
{
    ...
    /// 使い方:
    /// <list type="number">
    ///   <item>SampleScene に本コンポーネントを持った GameObject を配置。</item>
    ///   <item><see cref="_facialController"/> に HatsuneMiku の FacialController を参照。</item>
    ///   <item>Play を押すと画面左上に HUD が表示される。</item>
    ///   <item>Controller / Keyboard それぞれのウェイトスライダを調整し、各ソースの
    ///   TriggerOn/Off ボタンで Expression を押してブレンド結果を目視する。</item>
    /// </list>
    /// <para>
    /// 本 HUD は Samples 固有のデモツールであり、パッケージ本体 (Runtime) には含めない。
    /// FacialController の既存公開 API (<c>SetInputSourceWeight</c> /
