...
namespace Hidano.FacialControl.Adapters.Json
{
    ...
    /// schemaVersion <c>"2.1"</c> 以外を <see cref="Debug.LogError(object)"/> +
    /// <see cref="NotSupportedException"/> で拒否する（Req 10.1）。
    /// </para>
    /// </summary>
    public sealed class SystemTextJsonParser : IJsonParser
    {
        /// <summary>
        /// 中間 JSON Schema v2.1 の strict version 文字列。
        /// </summary>
        public const string SchemaVersionV2 = "2.1";
        ...
        private static ProfileSnapshotDto ParseProfileSnapshotV2Internal(string json, out string preprocessed)
        {
            ...
            {
                throw new FormatException("プロファイル JSON (v2.1) のパースに失敗しました。", ex);
            }

            if (dto == null)
                throw new FormatException("プロファイル JSON (v2.1) のパースに失敗しました。結果が null です。");
            ...
            if (string.IsNullOrEmpty(dto.schemaVersion) || dto.schemaVersion != SchemaVersionV2)
            {
                ...
                Debug.LogError(
                    $"SystemTextJsonParser: 中間 JSON schema v2.1 の strict チェックに失敗しました。" +
        ...
        /// <summary>
        /// 既に組み立て済みの <see cref="ProfileSnapshotDto"/> を schema v2.1 互換 JSON 文字列にシリアライズする。
