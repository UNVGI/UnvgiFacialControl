...
- `OscAdapterBinding` の受信 mapping を BlendShape 専用の旧構造から、`OscMappingEntry` / `OscMappingMode` による mode 別 entry 構造へ破壊的に変更しました。既存の `OscAdapterBinding` Asset / Scene は preview 段階の方針に従い自動 migration を提供しないため、新しい `_mappings` に再設定してください。
- Gaze 受信を `OscAdapterBinding` に統合し、`Gaze_VRChat_XY` と `Gaze_ARKit_8BS` の mode を追加しました。ARKit 8 BlendShape mode では `addressPattern` を無視し、固定の `/ARKit/eyeLook...` 8 アドレスから左右別 Vector2 を復元します。
...
- `OscAdapterBinding` に bundle atomic swap、staleness fail-safe、sender identity によるゾンビ排除、heartbeat 整合性検査、Gaze Vector2 受信 source 登録を追加しました。
- `OscSenderOptionsDto` / `OscSenderEndpointDto` / `OscReceiverOptionsDto` / `OscMappingEntryDto` と JSON スキーマドキュメントを追加しました。
- UI Toolkit ベースの `OscSenderAdapterBindingDrawer` と `OscAdapterBindingDrawer` を追加し、送受信設定と mode 別 mapping を Inspector から編集できるようにしました。
...
- `OscAdapterBinding`、`ArKitOscAdapterBinding`、`OscReceiverHost`、`OscSenderHost`、`OscInputSource`、`OscDoubleBuffer`、`OscMappingTable` を追加しました。
