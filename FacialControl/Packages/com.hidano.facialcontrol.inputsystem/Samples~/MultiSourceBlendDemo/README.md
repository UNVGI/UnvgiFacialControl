...
単一レイヤーに `input` (InputSystem の Trigger Action 経由で controller / keyboard 双方を受ける ExpressionTrigger 入力源) を配置してウェイトブレンドの振る舞いを確認するとともに、Vector2 入力 (左スティック / WASD) で両目を同時駆動するアナログ操作 (目線) の挙動を目視するための PlayMode 専用サンプルです。
...
- **Character** — モデルを配置するルート GameObject
  ...
  - `InputFacialControllerExtension`（`input` 予約 ID を InputSourceFactory に登録）
...
- `Input weight` スライダーで `input` ソースの重みを 0〜1 で調整
- `入力源` ブロックで smile / anger / surprise / lipsync_a の **On / Off** をトリガー
- **コントローラ / キーボード** いずれの入力でも同じ `input` ソースに集約され、Action 名 (Trigger1〜4) → Expression ID マッピングで発火します
- **キーボード 1 / 2 / 3 / 4** キーでも `input` 経由で smile / anger / surprise / lipsync_a が発火します（SO の `ExpressionBindings` セクションで Action 名 → Expression ID をマッピング）
...
- **`input は profile.inputSources に未宣言`**: `StreamingAssets/FacialControl/MultiSourceBlendDemoCharacter/profile.json` が `Assets/StreamingAssets/...` 配下に展開されているか、SO の `_layers[].inputSources` に `input` ソースが宣言されているかを確認
