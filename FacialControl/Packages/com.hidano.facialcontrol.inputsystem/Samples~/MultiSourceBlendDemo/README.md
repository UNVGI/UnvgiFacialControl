...
同一レイヤーに `controller-expr` と `keyboard-expr` の 2 つの ExpressionTrigger 入力源を並置してウェイトブレンドの振る舞いを確認するとともに、Vector2 入力 (左スティック / WASD) で両目を同時駆動する EyeLook 表情の挙動を目視するための PlayMode 専用サンプルです。
...
| `MultiSourceBlendDemoCharacter.asset` | `FacialCharacterSO`（schema v2.0、表情 5 種 (smile / anger / surprise / lipsync_a + EyeLook) と Trigger1〜10 + Look (Vector2) のバインディングを内蔵） |
| `MultiSourceBlendDemoActions.inputactions` | Expression 用 Trigger1〜10 + EyeLook 用 Look (Vector2) を 1 つの ActionMap "Expression" にまとめた InputActionAsset |
...
| `MultiSourceBlendDemoHUD.cs` | Weight スライダー + 各ソースごとの TriggerOn/Off ボタン + EyeLook 関連 BlendShape の現在値を表示する HUD コンポーネント |
...
- **左スティック** または **WASD** で `Look` (Vector2) を入力すると、SO の Inspector で EyeLook 表情に設定した両目 BlendShape が同時に駆動されます。BlendShape 名はモデルに合わせて Inspector で編集してください
