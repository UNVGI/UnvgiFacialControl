...
- [ ] 1. プロジェクト基盤と early-fail 検証スモークテストを整備する
- [ ] 1.1 VContainer scoped registry を `manifest.json` に追加し asmdef references を更新する
...
- [ ] 1.2 R-A: VContainer Unity 6 互換 smoke test を EditMode に追加する
...
- [ ] 1.3 R-B: `[SerializeReference]` round-trip smoke test を EditMode に追加する
...
- [ ] 2. Domain 層の値オブジェクトと抽象基底を実装する
- [ ] 2.1 (P) `AdapterSlug` 値オブジェクトの EditMode テストを書く（Red）
...
- [ ] 2.2 `AdapterSlug` 値オブジェクトを実装し Red を Green にする
...
- [ ] 2.3 (P) `AdapterBindingBase` 抽象基底と `FacialAdapterBindingAttribute` を Domain 層に実装する
...
- [ ] 3. Adapters 層の DI / Lifecycle / Registry を実装する
- [ ] 3.1 (P) `AdapterBuildContext` readonly struct を実装する
...
- [ ] 3.2 `AdapterBindingHost` の EditMode テストを書く（Red）
...
- [ ] 3.3 `AdapterBindingHost` を実装し Red を Green にする
...
- [ ] 3.4 (P) `IInputSourceRegistry` / `InputSourceRegistry` の EditMode テストを書く（Red）
...
- [ ] 3.5 `IInputSourceRegistry` / `InputSourceRegistry` を実装し Red を Green にする
...
- [ ] 3.6 `FacialControlAppLifetimeScope` / `FacialControllerLifetimeScope` を実装する
...
- [ ] 4. `FacialCharacterProfileSO` に AdapterBindings field を追加する
- [ ] 4.1 SO 修正の round-trip テストを EditMode に書く（Red）
...
- [ ] 4.2 `FacialCharacterProfileSO` を修正して field を追加し Red を Green にする
...
- [ ] 5. Editor 層の Discovery / ListView / SaveGuard を実装する
- [ ] 5.1 `AdapterBindingDiscovery` の EditMode テストを書く（Red）
...
- [ ] 5.2 `AdapterBindingDiscovery` を実装し Red を Green にする
...
- [ ] 5.3 `AdapterBindingsListView` の EditMode テストを書く（Red）
...
- [ ] 5.4 `AdapterBindingsListView` / `AdapterBindingAddDropdown` / `MissingAdapterPlaceholderElement` を実装し Red を Green にする
...
- [ ] 5.5 (P) `FacialCharacterProfileAssetGuard` を実装する
...
- [ ] 5.6 `FacialCharacterProfileSOInspector` に AdapterBindings セクションを追加する
...
- [ ] 6. `FacialController` を Phase 1 並走仕様で改修する
- [ ] 6.1 `FacialController` Phase 1 並走仕様の PlayMode テストを書く（Red）
...
- [ ] 6.2 `FacialController.Initialize` / `Cleanup` に VContainer child scope build を追加し Red を Green にする
...
- [ ] 7. Phase 1 PlayMode 統合テストと Performance テストを追加する
- [ ] 7.1 (P) `AdapterBindingHostLifecycleTests` を実装する
...
- [ ] 7.2 (P) `AdapterBindingHostAllocationTests` を 3 シナリオ分解で実装する
...
- [ ] 7.3 (P) `MultiSourceBlendThreeBindingsTests` を実装する
...
- [ ] 8. core 同梱サンプル `MultiSourceBlendBasicSample` を整備する
- [ ] 8.1 Mock binding と Runner を実装する
...
- [ ] 8.2 `package.json` の `samples[]` に Sample を登録する
...
- [ ] 9. OSC adapter package を新アーキテクチャに移行する
- [ ] 9.1 `OscAdapterBinding` の EditMode / PlayMode テストを書く（Red）
...
- [ ] 9.2 `OscReceiverHost` / `OscSenderHost` helper MonoBehaviour と `OscAdapterBinding` を実装する
...
- [ ] 9.3 (P) `ArKitOscAdapterBinding` を実装する
...
- [ ] 9.4 (P) `OscAdapterBindingDrawer` / `ArKitOscAdapterBindingDrawer` を提供する
...
- [ ] 10. InputSystem adapter package を新アーキテクチャに移行する
- [ ] 10.1 `InputSystemAdapterBinding` の EditMode / PlayMode テストを書く（Red）
...
- [ ] 10.2 `InputSystemAdapterBinding` を実装し旧 `FacialCharacterInputExtension` ロジックを移植する
...
- [ ] 10.3 (P) `InputSystemAdapterBindingDrawer` を提供する
...
- [ ] 10.4 InputSystem package の旧資産を削除する
...
- [ ] 10.5 inputsystem `Samples~/MultiSourceBlendDemo/` HUD を slug 駆動に書き換える
...
- [ ] 11. Core 側の旧資産を Phase 2 で削除する
- [ ] 11.1 `IFacialControllerExtension` 関連を core から削除する
...
- [ ] 11.2 `InputSourceFactory` / `InputSourceId.ReservedIds` 旧資産を削除する
...
- [ ] 12. Phase 2 検証 Checkpoints と Documentation を整備する
- [ ] 12.1 (P) `OscAdapterBindingIntegrationTests` を Phase 2 完了後の構成で実行する
...
- [ ] 12.3 (P) 単一 SO に Input + OSC + ARKit 同時保持の round-trip 統合テストを追加する
...
- [ ] 12.5 CHANGELOG と migration-guide を整備する
