...
**新規コンポーネント**: `OscSenderOptionsDto`、`OscReceiverOptionsDto`（allowlist 削除済み）、`OscGazeReceiverOptionsDto`。
...
**新規コンポーネント**: `OscSenderAdapterBindingDrawer`、拡張版 `OscAdapterBindingDrawer`、`OscGazeReceiverAdapterBindingDrawer`。
...
### Requirement 12: Gaze Vector2 受信 binding

**既存資産**: `IAnalogInputSource` / `InputSourceRegistry.Register(slug, sub, source)` で `slug:sub` 形式の合成キー登録が可能。`ArKitOscAnalogSource` の subscribe + `pendingValues` 蓄積パターンを 8 名に限定して流用可能。

**新規コンポーネント**: `OscGazeReceiverAdapterBinding`、`GazeVector2InputSource`（左目用 / 右目用ペアを生成する factory）。

**複雑度**: M、**リスク**: Medium（R8: 受信元 binding の依存解決順序）
...
### R8: 受信側依存解決順（Req 12） — Design で確定
`OscGazeReceiverAdapterBinding` が `OscAdapterBinding` の listen endpoint / `OscReceiver` を参照したいが、binding 間の `OnStart` 順序は SO 上のリスト順依存。設計フェーズで「`AdapterBuildContext` 経由で receiver host を遅延 resolve する DI 機構」か「同 child scope の他 binding を slug で参照する公開 API」かを確定要。
