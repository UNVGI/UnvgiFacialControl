...
- [ ] 1. S-10: OSC 受信側 ContributeMask / TryWriteValues の mesh-index 空間整合
- [ ] 1.1 IInputSource.ContributeMask の mesh-index 空間契約を XML doc-comment に明文化する
...
- [ ] 1.2 HeartbeatConsistencyChecker を mesh BlendShape 数長で構築するテストを追加する（Red）
...
- [ ] 1.3 HeartbeatConsistencyChecker を mesh BlendShape 数長で構築するよう実装する（Green）
...
- [ ] 1.4 OscInputSource の mesh-index 空間書込みテストを新規追加する（Red）
...
- [ ] 1.5 OscInputSource.TryWriteValues を mesh-index 空間書込みに書き換える（Green）
...
- [ ] 1.6 OscAdapterBinding.OnStart で mesh-index 空間整列経路を結線する
...
- [ ] 1.7 OscReceiverDemo + OscOutputDemo 同時起動の例外消失を EditMode で再現検証する
...
- [ ] 2. S-11: Expression 遷移メタの AnimationClip ベイク撤去
- [ ] 2.1 AnimationClipExpressionSampler のメタ既定値返却テストを更新する（Red）
...
- [ ] 2.2 ExpressionClipBakery.Bake から AnimationEvent 書き込みを撤去する（Green）
...
- [ ] 2.3 AnimationClipExpressionSampler.SampleSummary をメタ無し既定値返却に統一する（Green）
...
- [ ] 2.4 ExpressionCreatorWindow から「遷移メタデータ」Foldout を削除する
...
- [ ] 2.5 FacialCharacterProfileExporter の SO 真値経路を回帰確認する
...
- [ ] 3. S-12: ExpressionCreatorWindow UI 改善
- [ ] 3.1 ベイクボタン潰れ防止の EditMode テストを追加し flexShrink / minWidth を適用する
  ...
  - _Boundary: ExpressionCreatorWindow, ExpressionCreatorWindowTests_
- [ ] 3.2 PreviewRenderWrapper に PNG 出力用 Texture2D 取得 API を追加する
...
- [ ] 3.3 ExpressionCreatorWindow leftPanel に PNG 書き出しボタンを追加する
  ...
  - _Boundary: ExpressionCreatorWindow, ExpressionCreatorWindowTests_
- [ ] 3.4 ExpressionCreatorWindow に AnimationClip 新規作成ボタンを追加する
...
- [ ] 4. S-13: Expression Row の Gaze 用 AnimationClip スロット非表示
- [ ] 4.1 isGaze 値に応じた AnimationClip スロット表示制御の EditMode テストを追加する（Red）
...
- [ ] 4.2 FacialCharacterProfileSOInspector の Expression Row clip スロット表示制御を実装する（Green）
...
- [ ] 5. S-14: GazeConfigs 一括再生成 + Undo 経路復元
- [ ] 5.1 GazeConfig 削除 Undo 経路の EditMode テストを追加する（Red）
...
- [ ] 5.2 RemoveGazeConfigAt に Undo.RecordObject を挿入する（Green）
  ...
  - _Boundary: FacialCharacterProfileSOInspector_
- [ ] 5.3 GazeConfigs 一括再生成ロジックの EditMode テストを追加する（Red）
...
- [ ] 6. S-15: Gaze 左右別 Action UI 配置見直し
- [ ] 6.1 Expression Binding 行の並び順 + Action 名 enable 状態の EditMode テストを更新する（Red）
...
- [ ] 6.2 InputSystemAdapterBindingDrawer の並び順と Action 名 enable 制御を実装する（Green）
...
- [ ] 7. S-16: uLipSync PhonemeEntry Undo 表示整合
- [ ] 7.1 PhonemeEntryListView の Undo 後表示整合 EditMode テストを追加する（Red）
...
- [ ] 7.2 PhonemeEntryListView に Undo.undoRedoPerformed 購読 / 解除を実装する（Green）
...
- [ ] 8. 仕上げ: backlog 整理とリリースノート更新
- [ ] 8.1 docs/backlog.md から S-10〜S-16 エントリを削除する
...
- [ ] 8.2 preview.1 リリースノートと CHANGELOG / README に破壊的変更を反映する
