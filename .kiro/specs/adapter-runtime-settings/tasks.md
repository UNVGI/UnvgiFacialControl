...
- [ ] 0. design.md と backlog の整合性修正
- [ ] 0.1 design.md の `AdapterRuntimeSettingsBase` 配置記述を Adapters 層に統一する
  ...
  - _Requirements: 2.1_
- [ ] 0.2 docs/backlog.md に「sub-asset 削除時の逆引き確認ダイアログ」項目を追加し、本 spec 内では Drawer HelpBox 警告で代替する方針を明文化する
...
- [ ] 1. Adapter Runtime Settings のパッケージ配置と asmdef を準備する
- [ ] 1.1 Core パッケージに RuntimeSettings 用ディレクトリと asmdef 参照を整える
...
- [ ] 1.2 OSC / LipSync パッケージ側に RuntimeSettings / Devices ディレクトリを整える
...
- [ ] 1.3 LipSync テスト用 InternalsVisibleTo と PlayerPrefs Fake 用基盤を整える
...
- [ ] 2. 親 SO + Sub-asset 共通基底の構築
- [ ] 2.1 AdapterRuntimeSettingsBase に `_label` / `_schemaVersion` と ToJson/FromJson 仮想 API を実装する
...
- [ ] 2.2 AdapterRuntimeSettingsCollectionSO に List<Base> と TryFind API を実装する
  ...
  - _Depends: 2.1_
- [ ] 2.3 Collection の sub-asset 操作不変条件と null 要素警告を実装する
...
- [ ] 3. OscRuntimeSettingsSO の実装とラウンドトリップ対応
- [ ] 3.1 (P) OscRuntimeSettingsSO に Receiver / Sender セクション両方のフィールドと getter を実装する
  ...
  - _Depends: 2.1_
- [ ] 3.2 (P) `_receiverEnabled` / `_senderEnabled` トグルと OnAfterDeserialize 正規化を実装する
  ...
  - _Boundary: OscRuntimeSettingsSO_
  - _Depends: 2.1_
- [ ] 3.3 OscRuntimeSettingsSO の ToJson/FromJson を override し JSON ラウンドトリップを実装する
...
- [ ] 4. PlayerPrefs ベースの DeviceStore を実装する
- [ ] 4.1 (P) IPlayerPrefsBackend と DefaultPlayerPrefsBackend を実装する
  ...
  - _Depends: 1.3_
- [ ] 4.2 (P) FakePlayerPrefsBackend を Shared テスト基盤に実装する
  ...
  - _Depends: 1.3_
- [ ] 4.3 LipSyncDeviceStore の静的 Load/Save API と Backend 差し替えフックを実装する
...
- [ ] 5. AdapterBinding を SettingsSO / DeviceStore 参照経路に切り替える
- [ ] 5.1 (P) OscAdapterBinding から環境依存フィールドを削除し OscRuntimeSettingsSO 参照に切り替える
  ...
  - _Depends: 3.3_
- [ ] 5.2 (P) OscSenderAdapterBinding から環境依存フィールドを削除し OscRuntimeSettingsSO 参照に切り替える
  ...
  - _Depends: 3.3_
- [ ] 5.3 (P) ULipSyncAdapterBinding から `_deviceDescriptor` SerializeField を削除し DeviceStore 経由読み込みに切り替える
...
- [ ] 6. Inspector / Editor 拡張の整備
- [ ] 6.1 (P) OscAdapterBindingDrawer / OscSenderAdapterBindingDrawer を改修して SettingsSO ObjectField に差し替える
  ...
  - _Depends: 5.1, 5.2_
- [ ] 6.2 (P) ULipSyncAdapterBindingDrawer から `_deviceDescriptor` UI を取り除き DeviceStore 連動の DeviceDescriptorPopup を結線する
  ...
  - _Depends: 5.3_
- [ ] 6.3 AdapterRuntimeSettingsTypeRegistry を Editor 専用 helper として実装する
  ...
  - _Depends: 2.1, 3.1_
- [ ] 6.4 AdapterRuntimeSettingsCollectionEditor (UI Toolkit) を実装し sub-asset 追加/削除 UI を提供する
...
- [ ] 7. 既存 Asset とテストフィクスチャの新構造移行
- [ ] 7.1 既存 `*Profile.asset` と Samples~ 内の fixture を新構造に再構築する
...
- [ ] 8. テストカバレッジを新構造に揃える
- [ ] 8.1 (P) AdapterRuntimeSettingsCollectionSO の sub-asset 追加/削除でパラメータ消失が無いことを検証する EditMode テストを追加する
  ...
  - _Depends: 2.3_
- [ ] 8.2 (P) OscRuntimeSettingsSO の Serialize/Deserialize と JSON ラウンドトリップ EditMode テストを追加する
  ...
  - _Depends: 3.3_
- [ ] 8.3 (P) LipSyncDeviceStore の Fake backend 経由 EditMode テストを追加する
  ...
  - _Depends: 4.3_
- [ ] 8.4 (P) AdapterRuntimeSettingsCollectionEditor の Add/Remove と _label 重複 warning を EditMode (Editor) テストで検証する
  ...
  - _Depends: 6.4_
- [ ] 8.5 (P) OscAdapterBinding / OscSenderAdapterBinding が SettingsSO 経由で UDP 送受信できることを PlayMode で検証する
  ...
  - _Depends: 5.1, 5.2_
- [ ] 8.6 (P) ULipSyncAdapterBinding が DeviceStore 経由 DeviceName でマイク初期化フローを開始することを PlayMode で検証する
  ...
  - _Depends: 5.3_
- [ ] 8.7 既存テスト群の旧フィールド参照を全て新構造に書き換える
...
- [ ] 9. ドキュメントと backlog を最終化する
- [ ] 9.1 Documentation~ にマイグレーションと運用ガイドを追記する
...
- [ ] 9.2 docs/backlog.md にスコープ外項目と将来マイグレーションを記録する
